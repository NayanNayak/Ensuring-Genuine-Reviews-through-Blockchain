...
const Get = async (req, res, next) => {
  ...
  try {
    ...
    // Ensure the overall rating is up to date
    await updateProductRating(product_id);
    
    // Fetch the product again with the updated rating
...
const GetRating = async (req, res, next) => {
  ...
  try {
    ...
    // Get the product with rating information
    const product = await Product.findById(product_id).select('overall_rating review_count title');
